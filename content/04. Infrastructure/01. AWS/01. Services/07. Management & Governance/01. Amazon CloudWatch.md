...
`total_fail == topo_fail` 이면 그 서비스의 스케줄 실패가 100% topology 때문이라는 뜻이다. 작은 서비스(예: 200m/1Gi)도 막혔다면 리소스가 아니라 [[03. Kubernetes Scheduling#Pod Topology Spread Constraints|topology]] 가 주범임을 입증한다.
...
> 이 쿼리들로 4시간 지연의 원인을 topology 로 특정한 사례: [[03. Amazon EKS#EKS 노드 그룹 업그레이드 지연 사례]]. 노드그룹 단위 launch/terminate 타임라인은 audit 가 아니라 ASG `describe-scaling-activities` 로 본다(그쪽 노트 참조).
