...
### 물리 구성
```
   On-Premises DC            DX Location (3rd party IDC)
┌──────────────────┐      ┌───────────────────────────────────┐
│                  │      │  ┌───────────┐   ┌─────────────┐  │
│ Customer Router  │      │  │ Customer  │   │  AWS Cage   │  │
│      (CGW)       │──────┼─▶│   Cage    │──▶│  DX Router  │  │
│                  │      │  │  Router   │   │             │  │
└──────────────────┘      │  └───────────┘   └──────┬──────┘  │
                          │     Cross-Connect       │         │
                          └─────────────────────────┼─────────┘
                                        ...
                                        BGP Peering │
                                          ...
                                          └─────────────────────┘
```
- 고객은 DX Location 의 자기 Cage 또는 파트너 Cage 까지 회선을 끌어온다
- Cross-Connect = DX Location 건물 안에서 고객 라우터 포트와 AWS 라우터 포트를 광케이블로 물리 연결하는 작업
- 물리 연결 위에 BGP Peering 을 맺어야 비로소 논리적인 경로가 생긴다
- 연결 방식
	- Dedicated Connection: 물리 포트를 단독으로 사용, 1/10/100 Gbps
	- Hosted Connection: 파트너가 보유한 포트를 분할받아 사용, 50 Mbps ~ 10 Gbps

### 논리 구성, Virtual Interface (VIF)
하나의 물리 연결 위에 용도별 VIF 를 올려 퍼블릭 리소스와 프라이빗 리소스에 동시에 접근한다.

```
                      DX Connection (BGP Peering)
                                 │
        ┌────────────────────────┼────────────────────────┐
        │                        │                        │
   ...
   └─────────┘              └─────────┘
```
- Private VIF: 프라이빗 IP 로 VPC 에 접근한다. VGW 에 직접 연결하거나 DXGW 를 거친다
- Transit VIF: DXGW 를 거쳐 TGW 에 연결해 다수의 VPC 에 접근한다
- Public VIF: 퍼블릭 IP 를 사용하는 AWS 공용 서비스에 접근한다. DXGW 를 사용할 수 없다
