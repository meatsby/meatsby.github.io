---
title: 03. Kubernetes Scheduling
...
쿠버네티스에서 파드를 특정 노드에 배치할 때 사용하는 `nodeName` 과 `nodeSelector` 의 가장 핵심적인 차이는 '쿠버네티스 스케줄러(Scheduler)의 개입 여부'와 '지정 방식'이다. `nodeName` 은 스케줄러를 완전히 건너뛰고 특정 노드에 파드를 강제로 직접 바인딩하는 반면, `nodeSelector` 는 스케줄러가 노드의 레이블(Label)을 평가하여 조건에 맞는 노드를 선택하도록 유도한다. 다만 `nodeSelector`  단순히 Label 만 확인하기 때문에 더 복잡한 조건으로 Node 를 설정하고 싶을 때 Node Affinity 를 활용할 수 있다.
...
현재 2가지가 제공되고 추후 2가지가 더 추가될 예정이다.
해당 설정은 이름 그대로 작동하는데,
- `DuringScheduling` 은 Pod 가 Schedule 되는 시점에 Node 에 알맞는 label 이 있는지 확인하는 것이고
- `DuringExecution` 은 Node 가 Pod 를 호스팅하는 중 label 이 변경되었을 시 처리를 지정하는 것이다.
...
`ResourceQuota` 를 생성하여 `namespace` 전체에 리소스 요청 및 제한을 설정할 수 있다.
...
metadata:
  name: high-priority
value: 1000000  # default value 는 0 이다.
description: "This priority class should be used for XYZ service pods only."
globalDefault: false
preemptionPolicy: PreemptLowerPriority # default 설정 'PreemptLowerPriority | Never'
...
spec:
  ...
  - name: nginx
    ...
    ports:
    - containerPort: 8080
    priorityClassName: high-priority  # PriorityClass 지정
```

`PriorityClass` 는 `Pod` 의 우선순위를 정해주는 방법이다.
- 쿠버네티스 컴포넌트와 같이 중요한 `Pod` 일 경우: 2,000,000,000 ~ 1,000,000,000 범위를 할당하고,
- 그 외 `Pod` 는 1,000,000,000 ~ -2,147,483,648 범위 내에서 우선순위를 할당한다.

클러스터에 리소스가 부족한 경우 `preemptionPolicy` default 설정인 `PreemptLowerPriority` 로 인해 낮은 priority 를 가진 Pod 이 제거되고 그 자리를 차지하게 된다. 이를 방지하기 위해 `never` 옵션을 줄 수도 있다.
...
> 노드 업그레이드 중 이 제약(`DoNotSchedule`, `maxSkew: 1`)이 교체 파드를 Pending 으로 묶어 4시간 지연을 일으킨 사례: [[03. Amazon EKS#EKS 노드 그룹 업그레이드 지연 사례]]

## `0/N nodes available` 스케줄러 디버깅
---
파드가 `Pending` 이면 `kubectl describe pod` 의 FailedScheduling 이벤트를 읽는다. 스케줄러는 모든 노드를 **필터(predicate) 순서대로** 검사해 탈락 사유별로 노드를 분류하고, 그 합계를 메시지로 뱉는다.

```
0/28 nodes are available:
 1 Insufficient cpu,
...
사유별로 노드의 "상태"가 다르다는 게 핵심이다.

- `unschedulable` = **cordon** 된 노드(업그레이드로 drain 대기/완료). 후보에서 제외.
- `untolerated taint(s)` = 다른 노드그룹이거나 NotReady 등 taint 가 걸린 노드. 후보 제외.
- `Insufficient cpu`/`memory` = 리소스 부족. 진짜로 자리가 없는 노드.
- `didn't match pod topology spread constraints` = **cordon·taint·리소스 필터를 다 통과한 멀쩡한 노드**인데 [[03. Kubernetes Scheduling#Pod Topology Spread Constraints|topology]] 때문에 거부된 것. 즉 **"자리는 있는데 zone 이 틀린"** 노드다.
- `preemption: not helpful` = 우선순위 선점으로도 못 푼다(낮은 우선순위 파드를 쫓아내도 topology 는 안 풀리므로).

위 예시는 28노드 중 9(cordon)+4(taint)=13 이 후보에서 빠지고, 남은 노드 중 14 가 "정원 찬 zone" 이라 거부된 상황이다. 즉 **리소스가 아니라 topology 가 주범**이라는 판정이 메시지 한 줄에서 나온다.

> 이 메시지로 원인을 topology 로 특정한 실제 사례: [[03. Amazon EKS#EKS 노드 그룹 업그레이드 지연 사례]]
