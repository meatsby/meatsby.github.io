...
### QoS Class
쿠버네티스에서 [파드 서비스 품질(QoS) 클래스](https://kubernetes.io/ko/docs/concepts/workloads/pods/pod-qos/)는 노드(Node)에 자원 부족(메모리 부족 등) 상황이 발생했을 때, **어떤 파드(Pod)를 먼저 제거(Eviction)할지 결정**하는 기준이 됩니다. 세 가지 종류가 있습니다.
- **Guaranteed (보장됨)**
    - 모든 컨테이너에 대해 CPU와 메모리의 **requests와 limits이 모두 설정**되어 있고 값이 같을 때 부여됩니다.
    - 가장 우선순위가 높아 시스템 자원이 부족해도 **가장 나중에 축출**됩니다.
- **Burstable (변동 가능)**
    - 파드 내 컨테이너에 requests와 limits이 설정되어 있지만, **requests와 limits 값이 서로 다르거나** 일부만 설정된 경우입니다.
    - Guaranteed보다는 우선순위가 낮고 BestEffort보다는 높습니다.
- **BestEffort (최선 노력)**
    - CPU와 메모리에 대한 **requests나 limits이 전혀 설정되어 있지 않은** 경우입니다.
    - 자원이 부족할 때 **가장 먼저 축출**되는 대상입니다.
