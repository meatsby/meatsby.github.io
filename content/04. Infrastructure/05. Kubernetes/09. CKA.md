...
#### Review
- `/etc/kubernetes/manigests/kube-controller-manager.yaml`
	- POD CIDR `--cluster-cidr` = `k -n kube-system get configmap kubeadm-config -o yaml | awk '/podSubnet:/{print $2}'`
	- SVC CIDR `--service-cluster-ip-range` = `/etc/kubernetes/manifests/kube-apiserver.yaml | grep --service-cluster-ip-range`
