...
- **MTU (Maximum Transmission Unit)** = L2 frame 의 payload 최대 크기. 이더넷 기본 `1500` 바이트.
- **MSS (Maximum Segment Size)** = TCP segment 의 data 부분 최대 크기. **`MTU − IP 헤더(20) − TCP 헤더(20) = 1460`** 이 표준.
- 3-way handshake 때 양쪽이 자기 MSS 광고 → 둘 중 작은 값 채택.
- segment 가 MSS 보다 크면 TCP 가 split. packet 이 MTU 보다 크면 IP 가 fragment (성능 저하, 가능하면 회피).
