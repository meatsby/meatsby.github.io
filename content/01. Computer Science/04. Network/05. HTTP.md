...
- 서버의 **공개키 + 도메인 정보**를 담은 CA 서명 요청서이다
...
- `.crt` 의 내용은 **평문**이며, `Signature Value` 만 `ca.key` 로 암호화된 hash 이다
...
- CA (Certificate Authority) 는 **인증서 발급 시점**에만 관여하고, TLS 핸드셰이크 시점에는 참여하지 않는다.
...
### 인증서 폐지 확인 (OCSP / CRL)
- 인증서 서명 검증 자체는 **로컬에 저장된 CA 공개키**로 수행한다. (CA 와 실시간 통신 불필요)
...
- Kubernetes 에서 사용되는 인증서 파일은 모두 동일한 PEM 형식이지만, **용도와 서명 주체**가 다르다.
...
- **외부 통신 인증서** (`tls.crt`, `tls.key`)
    - `kubernetes.io/tls` 타입 Secret 에서 사용하는 **필드명 컨벤션**이다.
