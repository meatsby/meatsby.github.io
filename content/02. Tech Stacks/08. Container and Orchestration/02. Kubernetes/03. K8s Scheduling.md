...
> 노드 업그레이드 중 이 제약(`DoNotSchedule`, `maxSkew: 1`)이 교체 파드를 Pending 으로 묶어 4시간 지연을 일으킨 사례: [[06. K8s Cluster Maintenance#EKS 노드 그룹 업그레이드 지연 사례]]
...
- `didn't match pod topology spread constraints` = **cordon·taint·리소스 필터를 다 통과한 멀쩡한 노드**인데 [[03. K8s Scheduling#Pod Topology Spread Constraints|topology]] 때문에 거부된 것. 즉 **"자리는 있는데 zone 이 틀린"** 노드다.
...
> 이 메시지로 원인을 topology 로 특정한 실제 사례: [[06. K8s Cluster Maintenance#EKS 노드 그룹 업그레이드 지연 사례]]
