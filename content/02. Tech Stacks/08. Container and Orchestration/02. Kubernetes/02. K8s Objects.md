...
### k8s, 컨테이너, 파드는 결국 리눅스 프로세스다
- 쿠버네티스에서 파드(Pod)와 컨테이너(Container)는 실제로는 리눅스에서 실행되는 일반 프로세스(process)일 뿐이다.
- 컨테이너는 리눅스 커널의 여러 기능을 조합해 "격리된 환경에서 동작하는 프로세스"로 구현된다.

#### 리눅스에서 컨테이너를 구현하는 핵심 기술
- **네임스페이스(namespace)**: 프로세스가 자신만의 파일시스템, 네트워크, PID, 사용자, 호스트명 등을 갖는 것처럼 보이게 만드는 커널 기능. 각 컨테이너는 별도의 네임스페이스로 격리되어, 다른 컨테이너/호스트와 자원을 분리함.
  - 종류: mount, UTS, IPC, PID, network, user namespace 등
- **컨트롤 그룹(cgroup)**: 프로세스별로 CPU, 메모리, 디스크, 네트워크 등 리소스 사용량을 제한하고 모니터링할 수 있게 해주는 커널 기능. 컨테이너마다 cgroup이 할당되어 자원 사용을 제어함.
- **chroot 및 overlay 파일시스템**: 각 컨테이너가 독립적인 파일시스템을 갖는 것처럼 보이게 해줌. 실제로는 호스트의 파일시스템 일부를 격리하거나, 여러 계층의 파일시스템을 합쳐서 사용(overlayfs).
- **capabilities, seccomp, AppArmor/SELinux**: 보안 격리 및 권한 제한을 위해 사용되는 리눅스 커널 기능들.

#### 파드와 컨테이너의 관계
- 파드는 하나 이상의 컨테이너(프로세스 집합)를 묶은 논리적 단위일 뿐, 실제로는 같은 네임스페이스/네트워크/스토리지 등을 공유하는 리눅스 프로세스들의 집합이다.
- 즉, 쿠버네티스에서 컨테이너/파드를 생성하면 결국 리눅스 커널이 네임스페이스와 cgroup을 활용해 프로세스를 격리하고 관리하는 것에 불과하다.

> 요약: 쿠버네티스의 컨테이너, 파드는 "리눅스 커널의 네임스페이스와 cgroup 등 격리 기술을 활용해 구현된 리눅스 프로세스"이다. 컨테이너라는 특별한 것이 있는 게 아니라, 리눅스가 제공하는 격리/제어 기능을 조합해 만든 것일 뿐이다.
...
위 ClusterIP 예시로 들면, `back-end:8080` 으로 들어온 요청이 label 이 `app=myapp`, `type=back-end` 인 Pod 의 80 포트로 전달된다.
