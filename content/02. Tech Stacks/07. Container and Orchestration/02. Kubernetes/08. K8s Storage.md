...
spec:
  ...
  - name: data-volume
    hostPath:
      ...
      type: Directory
```
Pod 내 Container 에게 Volume 을 마운팅해주기 위해 위와 같이 설정할 수 있다. 위 내용은 Node 의 `/data` 디렉터리를 Volume 으로 사용하고 Container 내 `/opt` 디렉터리와 마운팅하겠다는 의미다.
...
- name: data-volume
  awsElasticBlockStore:
    ...
    fsType: ext4
```
Node 의 디렉터리를 Volume 으로 사용하는 것보다 EBS 등의 Storage 를 사용하는 것이 더 합리적이다.
