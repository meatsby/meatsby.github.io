...
→ telnet 의 #2 역할은 `nc` (netcat) 가 가져감. SSH 는 generic TCP client 가 아님. (자세한 nc 사용은 `09. 리눅스 네트워킹` 의 `### telnet — 포트 listen 확인` 참고.)
...
- 흔한 함정
    - 퍼미션 느슨 → `Permissions 0644 for 'mykey.pem' are too open.` → `chmod 400 mykey.pem`. `04. 파일 권한` 노트의 user 권한 규칙이 보안의 마지막 선.
